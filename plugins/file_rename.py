...
def extract_season_number(filename):
    # Try to extract season number from patterns that include season info
...
async def add_metadata(input_path, output_path, user_id):
    ...
    metadata = {
        ...
        'video_title': await madflixbotz.get_video(user_id),
        'audio_title': await madflixbotz.get_audio(user_id),
...
async def auto_rename_files(client, message):
    user_id = message.from_user.id
        ...
        if not format_template:
            return await message.reply_text("<b>⚠️ Didn't find any Rename Format ⁉️</b>\n<blockquote>➪ Plᴇᴀsᴇ, Sᴇᴛ An Auᴛᴏ Rᴇnᴀʍᴇ Fᴏrʍᴀᴛ Firsᴛ. Usᴇ **/autorename**</blockquote>")
        ...
        if episode_number:
            ...
            # Replace season placeholders if season number was found
            if season_number:
                season_placeholders = ["season", "Season", "SEASON", "{season}"]
                for placeholder in season_placeholders:
                    format_template = format_template.replace(placeholder, str(season_number), 1)
            ...
            download_msg = await message.reply_text(text="**➪ Tʀyɪɴɢ Tᴏ Dᴏᴡɴʟᴏᴀᴅɪɴɢ...**")
            try:
            ...
            try:
                ...
                await message.reply_text(f"⚠️ Failed to add metadata: {e}")

            upload_msg = await download_msg.edit("**➪ Tʀyɪɴɢ Tᴏ Uᴩʟᴏᴀᴅɪɴɢ...**")
            ph_path = None
            ...
            try:
                ...
                if type == "document":
                    await client.send_document(
                        ...
                        progress_args=("🗂️ Uᴘʟᴏᴀᴅ ᴛᴏ Tᴇʟᴇɢʀᴀᴍ ⌸ ➪", upload_msg, time.time())
                    )
                elif type == "video":
                    await client.send_video(
                        ...
                        duration=duration,
                        progress=progress_for_pyrogram,
                        progress_args=("📺 Uᴘʟᴏᴀᴅ ᴛᴏ Tᴇʟᴇɢʀᴀᴍ ⌸ ➪", upload_msg, time.time())
                    )
                elif type == "audio":
